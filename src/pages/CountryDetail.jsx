  ...
  const fetchMeals = async () => {
    try {
      ...
      const response = await axios.get(
        `https://www.themealdb.com/api/json/v1/1/filter.php?a=${encodeURIComponent(areaName)}`
      )
  ...
  return (
    <div className="w-full md:w-[95%] lg:w-[98%] mx-auto mt-8 mb-8">
      <div className="flex items-center gap-3 p-3 mb-4">
        ...
        <h2 className="lg:text-4xl text-2xl font-bold text-amber-800">{areaName} Recipes</h2>
      </div>
      <p className="text-amber-600 md:text-xl px-3 font-light mb-8">
        Discover delicious {areaName} meals from around the world. {meals.length > 0 && `Found ${meals.length} recipes.`}
      </p>
      ...
      <div className='mb-8 mt-5 w-[95%] md:w-[95%] lg:w-[98%] mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 gap-y-8 items-stretch auto-rows-fr justify-items-stretch'>
        ...
        {!loading && meals.length === 0 && !error && <p className="col-span-full text-center text-gray-500">No meals found for this country.</p>}
      </div>
    </div>
  )
}
